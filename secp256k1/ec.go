...
func EciesEncrypt(pubkey *PublicKey, in []byte) ([]byte, error) {
	ephemeral, err := NewPrivateKey(secp256k1Curve.S256())
	if err != nil {
		return nil, err
	}
...
func EciesDecrypt(priv *PrivateKey, in []byte) ([]byte, error) {
	...
	pubkey, err := ParsePubKey(pb, secp256k1Curve.S256())
	if err != nil {
		return nil, err
	}

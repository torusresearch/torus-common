...
func TestDeployNodelist(t *testing.T) {
	...
	//Deploy contract
	address, _, nodeListContract, err := nodelist.DeployNodeList(
		auth,
		blockchain,
	)
	...
	_, err = nodeListContract.EpochInfo(nil, big.NewInt(0))
	...
	// Epoch
	_, err = nodeListContract.UpdateEpoch(auth, big.NewInt(1), big.NewInt(5), big.NewInt(3), big.NewInt(1), []common.Address{}, big.NewInt(0), big.NewInt(2))
	if err != nil {
		t.Fatal(err)
	}
	blockchain.Commit()
	...
	for _, acc := range whitelistedAccounts {
		...
		_, err := nodeListContract.UpdateWhitelist(auth, big.NewInt(1), accAddress, true)
		if err != nil {
			t.Fatal(err)
		}
		blockchain.Commit()

		isWhitelisted, err := nodeListContract.IsWhitelisted(nil, big.NewInt(1), accAddress)
		if err != nil {
			t.Fatal(err)
		}
